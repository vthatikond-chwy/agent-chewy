...
When('I enter {string} into email field', async function (value) {
    const element = page.getByRole('textbox', { name: 'Email Address' });
    await element.waitFor({ state: 'visible', timeout: 10000 });
    await element.click();
...
When('I enter {string} into password field', async function (value) {
    const element = page.getByRole('textbox', { name: 'Password' });
...
When('I enter {string} into search field', async function (value) {
    const element = page.getByTestId('desktop-header').getByTestId('search');
    await element.waitFor({ state: 'visible', timeout: 10000 });
    await element.click();
    await element.fill(value);
...
When('I click on search button', async function () {
    const element = page.getByTestId('desktop-header').getByTestId('search-button');
    await element.waitFor({ state: 'visible', timeout: 10000 });
    await element.click();
    await page.waitForTimeout(500);
});
When('I click on product link', async function () {
    const element = page.getByRole('link', { name: 'Frisco Bungee Plush Squeaking' });
    await element.waitFor({ state: 'visible', timeout: 10000 });
    await element.click();
    await page.waitForTimeout(500);
});
...
//# sourceMappingURL=recording-1765149550238.steps.js.map