...
export function registerApiCommands(program) {
    ...
    apiCommand
        ...
        try {
            ...
            const orchestrator = new ApiTestOrchestrator(apiKey);
            const result = await orchestrator.generateForEndpoint(options.swagger, options.path, options.method);
            if (result.success) {
                console.log('\n✨ Success!');
    ...
    apiCommand
        ...
        try {
            ...
            const orchestrator = new ApiTestOrchestrator(apiKey);
            const result = await orchestrator.generateForTag(options.swagger, options.tag);
            if (result.success) {
                console.log('\n✨ Success!');
                console.log(`Generated ${result.generatedFiles.features.length} test files`);
            }
