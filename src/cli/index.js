...
apiCommand
    ...
    try {
        ...
        const spec = await SwaggerParser.dereference(options.swagger);
        const apiInfo = {
...
apiCommand
    ...
    try {
        ...
        const scenarios = await generator.generateTestScenarios(input, spec, options.endpoints, swaggerPath);
...
apiCommand
    ...
    try {
        ...
        const scenarios = await generator.generateTestScenarios(naturalLanguageInput, spec, [options.path], options.swagger);
        console.log(`✅ Generated ${scenarios.length} test scenarios\n`);
...
apiCommand
    ...
    try {
        ...
        console.log('🤖 Generating test scenarios...');
        const scenarios = await generator.generateTestScenarios(naturalLanguageInput, spec, taggedEndpoints, options.swagger);
        console.log(`✅ Generated ${scenarios.length} test scenarios\n`);
        // Extract team name from swagger spec path
        const teamName = extractProjectName(options.swagger);
        const apiTitle = spec.info?.title || '';
