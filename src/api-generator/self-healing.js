...
export class SelfHealing {
    ...
    constructor() {
        ...
        this.stepsDir = path.join(process.cwd(), 'src', 'steps', 'api');
    }
    /**
     * Run tests and detect undefined steps
     */
    async detectUndefinedSteps() {
        try {
            const { stdout, stderr } = await execAsync('npm run test:api 2>&1', { cwd: process.cwd() });
     ...
     */
    parseUndefinedStepsFromError(output) {
        const undefinedSteps = [];
     ...
     */
    async generateStepDefinition(step, featureContent) {
...
${step.stepType}('the response status code should be {int} for [scenario-name] test', function (this: CustomWorld, expectedStatusCode: number) {
  expect(this.response?.status).to.equal(expectedStatusCode);
     ...
     */
    getStepDefinitionFile(featureFilePath) {
     ...
     */
    async addStepDefinition(step, stepDefinitionCode) {
