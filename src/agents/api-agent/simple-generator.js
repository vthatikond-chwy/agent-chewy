...
export class SimpleTestGenerator {
     ...
     */
    generateRealisticTestData(fieldName, fieldSchema) {
     ...
     */
    parseExpectedResponseCode(naturalLanguage) {
     ...
     */
    parseInputDataFromNLP(naturalLanguage) {
        try {
            const inputData = {};
            let hasData = false;
     ...
     */
    extractProjectName(swaggerSpecPath) {
        try {
     ...
     */
    shouldUseOAuthToken(swaggerSpecPath, teamConfig) {
     ...
     */
    async generateTestScenarios(naturalLanguageInput, swaggerSpec, targetEndpoints, swaggerSpecPath) {
        // Parse exact input data from natural language command
        ...
        })).map(ep => `- ${ep.method} ${ep.path}\n  Summary: ${ep.summary}\n  Description: ${ep.description || 'N/A'}`).join('\n\n');
        ...
        if (teamRules?.responseCodeBehaviors) {
            ...
            const responseCodePatterns = Object.entries(behaviors).map(([code, config]) => {
  ...
  Endpoint: ${config.endpoint || 'N/A'}`;
            ...
            const scenarioMapping = Object.entries(behaviors).map(([code, config]) => {
                const triggers = config.triggers || [];
                return triggers.map((trigger) => `- For "${trigger}" scenarios → expect responseCode: "${code}", validatedAddress: ${config.validatedAddress}, requestAddressSanitized: ${config.requestAddressSanitized}`).join('\n');
            }).filter(Boolean).join('\n');
...
${exactCount ? `CRITICAL: Generate EXACTLY ${exactCount} test scenario${exactCount > 1 ? 's' : ''} as specified in the requirement. Focus only on the scenarios explicitly requested.` : 'Generate test scenarios based on the requirements provided.'}`;
        try {
            ...
            if (expectedResponseCode && scenarios.length > 0) {
                scenarios.forEach((scenario) => {
                    ...
                    scenario.__expectedResponseCode = expectedResponseCode;
                });
     ...
     */
    async generateCucumberFeature(scenario, swaggerSpec, swaggerSpecPath) {
...
${teamRules?.featureFileGeneration?.criticalRules ? `\n${teamRules.featureFileGeneration.criticalRules.map((r, idx) => `${idx + 9}. ${r}`).join('\n')}` : ''}
...
${rulesContext ? `\n\n${rulesContext}` : ''}`;
     ...
     */
    async generateStepDefinitions(cucumberFeature, scenario, swaggerSpec, swaggerSpecPath) {
...
CRITICAL: Make ALL step definitions unique by including scenario context in the step text to avoid conflicts with other test files.`;
        try {
            ...
            content = content.replace(/const\s+(\w+)\s*=\s*dataTable\.hashes\(\)\s*;\s*this\.requestBody\s*=\s*\1\s*;/g, 'const rows = dataTable.hashes();\n  const $1 = rows[0];\n  this.requestBody = $1;');
            ...
            content = content.replace(/Given\([^)]*headers[^)]*include[^)]*\)[^}]*\{[^}]*const\s+rows\s*=\s*dataTable\.hashes\(\)\s*;[^}]*for\s*\([^)]*\)\s*\{[^}]*this\.headers\[[^\]]+\]\s*=\s*rows\[[^\]]+\]\s*;[^}]*\}/g, (match) => {
                // Replace with correct pattern: extract first row and merge with existing headers
                return match.replace(/const\s+rows\s*=\s*dataTable\.hashes\(\)\s*;[^}]*for\s*\([^)]*\)\s*\{[^}]*this\.headers\[[^\]]+\]\s*=\s*rows\[[^\]]+\]\s*;[^}]*\}/, 'const rows = dataTable.hashes();\n  const headerRow = rows[0];\n  this.headers = { ...this.headers, ...headerRow };');
            });
            // Also fix simpler incorrect patterns for headers
            content = content.replace(/const\s+rows\s*=\s*dataTable\.hashes\(\)\s*;\s*for\s*\(const\s+key\s+in\s+rows\)\s*\{[^}]*this\.headers\[key\]\s*=\s*rows\[key\]\s*;[^}]*\}/g, 'const rows = dataTable.hashes();\n  const headerRow = rows[0];\n  this.headers = { ...this.headers, ...headerRow };');
            ...
            content = content.replace(/params:\s*\{\s*engine:\s*['"]default['"][^}]*\}/g, '// params removed - optional query parameters should not be included unless explicitly needed');
            // Also remove params object entirely if it only contains invalid values
            content = content.replace(/,\s*params:\s*\{\s*engine:\s*['"]default['"][^}]*\}\s*,?\s*/g, ' ');
            ...
            if (!isNegativeTest) {
                ...
                content = content.replace(/const\s+rows\s*=\s*dataTable\.hashes\(\);\s*\n\s*if\s*\(rows\s*&&\s*rows\.length\s*>\s*0\)\s*\{\s*\n\s*const\s+(\w+)\s*=\s*rows\[0\];\s*\n\s*this\.requestBody\s*=\s*\{([^}]*)\};\s*\n\s*\}\s*else\s*\{\s*\n\s*this\.requestBody\s*=\s*\{\};\s*\n\s*\}/g, 'const rows = dataTable.hashes();\n  const $1 = rows[0];\n  this.requestBody = {$2};');
            }
            ...
            content = content.replace(/`Bearer\s*`\s*\$\{process\.env\.([^}]*),/g, '`Bearer ${process.env.$1}');
            // Fix specific pattern: `Bearer ${process.env.OAUTH_TOKEN,` -> `Bearer ${process.env.OAUTH_TOKEN}`
            content = content.replace(/`Bearer\s*\$\{process\.env\.([^}]*),/g, '`Bearer ${process.env.$1}');
            // Fix duplicate Authorization headers and malformed structure
            // Pattern: 'Authorization': `Bearer ,\n    'Authorization': `Bearer ${process.env.OAUTH_TOKEN}`,
            content = content.replace(/'Authorization':\s*`Bearer\s*,\s*\n\s*'Authorization':\s*`Bearer\s*\$\{process\.env\.([^}]*)\}`/g, "'Authorization': `Bearer ${process.env.$1}`");
            // Fix malformed headers with extra closing braces and quotes
            // Pattern: 'x-submitter-id': '10'\n  }}\`\n  };
            content = content.replace(/('x-submitter-id':\s*'[^']*')\s*\}\}`\s*\n\s*\};/g, "$1\n  };");
            ...
            content = content.replace(/this\.headers\s*=\s*\{([^}]*)\$\{([^}]*),([^}]*)\}/g, (match, before, varName, after) => {
            ...
            const brokenHeadersPattern = new RegExp("this\\.headers\\s*=\\s*\\{'Content-Type':\\s*'application/json',\\s*'Authorization':\\s*`Bearer\\s*`\\s*\\$\\{process\\.env\\.([^}]*),\\s*'x-submitter-id':\\s*'([^']*)'\\}\\}`\\s*\\};", 'g');
            content = content.replace(brokenHeadersPattern, "this.headers = {\n    'Content-Type': 'application/json',\n    'x-submitter-id': '$2',\n    'Authorization': `Bearer ${process.env.$1}`\n  };");
            // Fix headers object with trailing commas and broken structure
            content = content.replace(/this\.headers\s*=\s*\{([^}]*'[^']*':\s*[^,}]*),([^}]*)\}/g, (match, before, after) => {
     ...
     */
    applyAllFixes(content, cucumberFeature, defaultHeaders, useOAuth, oauthTokenEnvVar, baseUrl) {
     ...
     */
    fixRequestBodyIssues(content, cucumberFeature) {
        ...
        content = content.replace(/this\.requestBody\s*=\s*\{([^}]*)\};\s*\}\s*else\s*\{/g, 'this.requestBody = {$1};\n  } else {');
        ...
        if (twoColumnTablePattern.test(cucumberFeature)) {
            ...
            content = content.replace(/const\s+rows\s*=\s*dataTable\.hashes\(\);\s*\n\s*const\s+(\w+)\s*=\s*rows\[0\];\s*\n\s*this\.requestBody\s*=\s*\{\s*(\w+):\s*\1\.(\w+)\s*\};/g, (match, varName, propName, propValue) => {
            ...
            const hasTwoColumnTable = cucumberFeature.match(/\|\s*\w+\s*\|\s*[^|]+\s*\|/);
            if (hasTwoColumnTable) {
                content = content.replace(/(Given\([^)]*request\s+body[^)]*\)[^}]*\{[^}]*const\s+rows\s*=\s*dataTable\.hashes\(\);\s*\n\s*const\s+\w+\s*=\s*rows\[0\];\s*\n\s*this\.requestBody\s*=\s*\{\s*\w+:\s*\w+\.\w+\s*\};)/g, (match) => {
                    // Replace hashes() with rowsHash() for 2-column tables
                    return match.replace(/const\s+rows\s*=\s*dataTable\.hashes\(\);\s*\n\s*const\s+(\w+)\s*=\s*rows\[0\];/, 'const $1 = dataTable.rowsHash();');
                });
            }
        }
     ...
     */
    fixHeadersObject(content, defaultHeaders, useOAuth, oauthTokenEnvVar, baseUrl) {
        ...
        return content.replace(endpointStepRegex, (match) => {
            ...
            const isMalformed = headersContent.includes('`Bearer ,') ||
     ...
     */
    fixDataTableParameters(content, cucumberFeature) {
     ...
     */
    fixQuotedStringPlaceholders(content, cucumberFeature) {
        ...
        for (const featureStep of featureSteps) {
            ...
            if (quotedStrings.length > 0) {
                ...
                const stepDefPattern = new RegExp(`(Given|When|Then|And|But)\\(['"]${escapedStepText}['"][^)]*\\)\\s*function\\s*\\(this:\\s*CustomWorld\\)\\s*\\{`, 'gi');
                // Store paramNames outside the replace callback so we can use them later
                let storedParamNames = [];
                content = content.replace(stepDefPattern, (match, stepType) => {
                    ...
                    for (const quotedStr of quotedStrings) {
                        fixedStepText = fixedStepText.replace(new RegExp(`["']${quotedStr.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}["']`, 'g'), '{string}');
                ...
                for (let i = 0; i < quotedStrings.length; i++) {
                    ...
                    const literalValuePattern = new RegExp(`(expect\\([^)]*\\)\\.to\\.(equal|include|have\\.property)\\(['"]${quotedValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]\\))`, 'gi');
        ...
        content = content.replace(/expect\\(this\\.response\\?\\.[^)]*\\)\\.to\\.(equal|include)\\(['"]application\/json['"]\\)/gi, (match) => {
     ...
     */
    removeDuplicateElseStatements(content) {
        ...
        while (content !== previousContent && iterations < 10) {
            ...
            content = content.replace(/(\};\s*)\s*\}\s*else\s*\{[^}]*\}\s*else\s*\{/g, '$1');
            // Pattern 4: Fix cases where else appears after closing brace of requestBody
            content = content.replace(/(this\.requestBody\s*=\s*\{[^}]*\};\s*)\s*\}\s*else\s*\{/g, '$1');
        }
        return content;
    }
    /**
     ...
     */
    finalHeadersFix(content, defaultHeaders, useOAuth, oauthTokenEnvVar) {
     ...
     */
    validateAndFixDataTablesInFeature(featureContent) {
     ...
     */
    fixEndpointSelection(content, scenario, swaggerSpec) {
        ...
        if (isValidationScenario && isUsingSuggest && swaggerSpec.paths) {
            const verifyEndpoint = Object.keys(swaggerSpec.paths).find(path => path.includes('verify') && !path.includes('Bulk'));
            if (verifyEndpoint) {
                ...
                if (verifySummary.toLowerCase().includes('validate') || verifySummary.toLowerCase().includes('verify')) {
                    ...
                    content = content.replace(new RegExp(`"/avs/v1.0/suggestAddresses"`, 'g'), `"${verifyEndpoint}"`);
                    content = content.replace(new RegExp(`/avs/v1.0/suggestAddresses`, 'g'), verifyEndpoint);
     ...
     */
    enforceWorkingTestData(content, scenario, teamRules) {
        if (!teamRules?.testData?.workingAddresses) {
            return content;
        }
        ...
        if (scenario.testData && Object.keys(scenario.testData).length > 0) {
            ...
            if (dataTableRegex.test(updatedContent)) {
                updatedContent = updatedContent.replace(dataTableRegex, `| streets | city | stateOrProvince | postalCode | country |\n      | ${testStreet} | ${testCity} | ${testState} | ${testPostal} | ${testCountry} |`);
            }
        ...
        if (isNegativeScenario && teamRules.testData.invalidAddresses) {
            ...
            if (scenario.endpoint.includes('verify') && !scenario.endpoint.includes('Bulk')) {
                ...
                if (scenarioNameLower.includes('zipcode') || scenarioNameLower.includes('postal') || contentLower.includes('zipcode')) {
                    ...
                    if (testData)
                        console.log('✅ Using default invalid test data for negative verifyAddress scenario');
                }
            }
            else if (scenario.endpoint.includes('suggest')) {
                // Check for specific invalid data types
                if (scenarioNameLower.includes('zipcode') || scenarioNameLower.includes('postal') || contentLower.includes('zipcode')) {
                    ...
                    if (testData)
                        console.log('✅ Using default invalid test data for negative suggestAddresses scenario');
                }
            }
        }
     ...
     */
    adjustExpectedResponseCodes(content, scenario, teamRules, swaggerSpec) {
        // HIGHEST PRIORITY: If user explicitly specified expected response code, DO NOT adjust it
        if (scenario.__expectedResponseCode) {
            console.log(`✅ Skipping response code adjustment - user specified: ${scenario.__expectedResponseCode}`);
            return content;
        }
        if (!teamRules?.testData?.workingAddresses) {
            return content;
        ...
        const isUsingWorkingData = (scenario.endpoint.includes('verify') && teamRules.testData.workingAddresses.verifyAddress) ||
     ...
     */
    fixResponseStructureHandling(content, endpoint, teamRules) {
        ...
        if (isArrayEndpoint) {
            ...
            content = content.replace(/const\s+responseData\s*=\s*this\.response\?\.[^;]+;\s*expect\(responseData\)\.to\.be\.an\(['"]array['"]\)/g, (match) => {
     ...
     */
    fixIncorrectPropertyPaths(content, endpoint, teamRules) {
     ...
     */
    ensureAllStepsGenerated(content, cucumberFeature, scenarioName) {
        // Extract all steps from feature file
  ...
  expect(${property}).to.be.${expectedValue};
     ...
     */
    writeFeatureFile(scenario, content, service, category) {
     ...
     */
    writeStepDefinitionFile(scenario, content, service) {
