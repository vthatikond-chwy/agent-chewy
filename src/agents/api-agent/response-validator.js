...
export class ResponseValidator {
     ...
     */
    generateValidation(responseSchema, statusCode, scenarioName) {
        return `
Then('the response matches the expected schema for ${statusCode} in ${scenarioName}', function(this: CustomWorld) {
  ...
  expect(valid).to.be.true;
    ...
    resolveSchema(schema, spec) {
        ...
        if (schema.allOf) {
            const resolved = { ...schema };
            resolved.allOf = schema.allOf.map((s) => this.resolveSchema(s, spec));
            return resolved;
        }
        if (schema.anyOf) {
            const resolved = { ...schema };
            resolved.anyOf = schema.anyOf.map((s) => this.resolveSchema(s, spec));
            return resolved;
        }
        if (schema.oneOf) {
            const resolved = { ...schema };
            resolved.oneOf = schema.oneOf.map((s) => this.resolveSchema(s, spec));
            return resolved;
        }
        return schema;
    }
     ...
     */
    extractResponseSchema(endpoint, statusCode, spec) {
