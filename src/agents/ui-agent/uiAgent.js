...
import { OptimizedHealingEngine } from '../healing/optimizedHealingEngine.js';
...
export class UIAgent {
    ...
    constructor(apiKey, options = {}) {
        ...
        if (this.options.useVision) {
            const screenshotDir = path.join(this.options.outputDir || 'test-results', 'screenshots');
    ...
    async initialize() {
        ...
        if (this.page) {
            ...
            this.healingEngine = new OptimizedHealingEngine(this.page, healingConfig, this.visionHealer || undefined);
        }
                ...
                for (let i = 0; i < scenario.steps.length; i++) {
                    ...
                    try {
                        ...
                        if (this.options.recordScreenshots && this.page) {
                            const screenshotPath = await this.takeScreenshot(`step-${result.stepsExecuted}`);
                            if (screenshotPath) {
                        ...
                        if (this.options.useVision &&
                            ...
                            try {
                                ...
                                const visionTime = ((Date.now() - visionStartTime) / 1000).toFixed(1);
                                console.error(`  ❌ Vision healing failed (${visionTime}s): ${visionError instanceof Error
                                    ? visionError.message
                                    : String(visionError)}`);
     ...
     */
    async healWithVision(step) {
        if (!this.page || !this.visionHealer || !this.locatorHelper) {
            throw new Error('Vision healing not available');
        }
        const visionResult = await this.visionHealer.findElementWithVision(this.page, step.target, step.description);
        ...
        if (step.action === 'click') {
            ...
            await this.locatorHelper.typeIntoElement(visionResult.selector, step.value, { useVision: false });
        }
    }
    /**
     * Takes a screenshot
