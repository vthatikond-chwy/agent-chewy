async;
generate(recording, RecordedSession);
...
generateSimpleFeature(recording, RecordedSession);
string;
{
    const lines = [
...
generateSimpleSteps(recording, RecordedSession);
string;
{
    const stepDefs = [
    ...
    recording.actions.forEach((action, index) => {
        if (action.type === 'click') {
            ...
            if (!generatedSteps.has(stepKey)) {
                ...
                stepDefs.push(`When('I perform click action {int}', async function(actionNum: number) {`, `  // Action ${index}: ${action.selector}`, `  const element = ${selector};`, `  await element.waitFor({ state: 'visible', timeout: 10000 });`, `  await element.click();`, needsWait
            ...
            if (!generatedSteps.has(stepKey)) {
                ...
                stepDefs.push(`When('I fill field {int} with {string}', async function(fieldNum: number, value: string) {`, `  // Action ${index}: ${action.selector}`, `  const element = ${selector};`, `  await element.waitFor({ state: 'visible', timeout: ${isPassword ? 20000 : 10000} });`, `  await element.click();`, `  await element.fill(value);`, `  await page.waitForTimeout(300);`, `});`, ``);
            }
        }
    });
...
export {};
//# sourceMappingURL=recordingToGherkin%20copy.js.map