...
When('I click on account link', async function() {
  const element = page.getByTestId('account-link').first();
...
When('I click on account button', async function() {
  const element = page.getByRole('button', { name: 'Account' });
  await element.waitFor({ state: 'visible', timeout: 10000 });
  await element.click();
  await page.waitForTimeout(500);
});

When('I click on sign out link', async function() {
  const element = page.getByRole('link', { name: 'Sign Out' });
  await element.waitFor({ state: 'visible', timeout: 10000 });
  await element.click();
  await page.waitForTimeout(500);
});
