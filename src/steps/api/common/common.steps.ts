 ...
 */
function resolveSchemaRefs(schema: any, spec: any, visited: Set<string> = new Set()): any {
  if (!schema) {
    return schema;
  }
 ...
 */
function findSwaggerSpec(baseUrl?: string, endpoint?: string): string | null {
  const possiblePaths = [
...
Then('the response matches the expected schema', async function (this: CustomWorld) {
  ...
  if (!valid && validate.errors) {
    ...
    const criticalErrors = validate.errors.filter((error: any) => {
    ...
    if (criticalErrors.length > 0 && criticalErrors.some((e: any) => e.keyword !== 'type' && e.keyword !==  'additionalProperties')) {
