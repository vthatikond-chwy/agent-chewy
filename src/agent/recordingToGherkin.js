export class RecordingToGherkinGenerator {
    ...
    generateSimpleFeature(recording) {
        const lines = [
    ...
    getClickDescription(action) {
        ...
        if (s.includes('search-button'))
            return 'search button';
        if (s.includes('add-to-cart'))
            return 'add to cart button';
        if (s.includes('proceed') || s.includes('checkout'))
            return 'proceed to checkout button';
        if (s.includes('credit') || s.includes('debit') || s.includes('card'))
        ...
        if (s.includes('link') && !s.includes('account'))
            return 'product link';
        return 'element';
    }
    getFillDescription(action) {
        const s = action.selector.toLowerCase();
        if (s.includes('email'))
            return 'email field';
        if (s.includes('password'))
    ...
    generateSimpleSteps(recording) {
        ...
        const hasOrderConfirmation = recording.actions.some(a => a.type === 'navigate' && a.url?.includes('thankyou'));
        if (hasOrderConfirmation && !generatedPatterns.has('order-confirmation')) {
            generatedPatterns.add('order-confirmation');
            stepDefs.push(`Then('I should see the order confirmation page', async function() {`, `  await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});`, `  const currentUrl = page.url();`, `  if (!currentUrl.includes('thankyou')) {`, `    throw new Error(\`Expected order confirmation page, but got \${currentUrl}\`);`, `  }`, `  console.log('✓ Order confirmed! URL:', currentUrl);`, `});`, ``);
        }
        recording.actions.forEach((action, index) => {
            switch (action.type) {
                case 'navigate':
                    if (!generatedPatterns.has('navigate')) {
                        ...
                        stepDefs.push(`Given('I navigate to {string}', async function(url: string) {`, `  await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });`, `  await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});`, `});`, ``);
                    }
                    break;
                case 'click':
                    const clickDesc = this.getClickDescription(action);
                    ...
                    if (!generatedPatterns.has(clickPattern)) {
                        ...
                        stepDefs.push(`When('I click on ${clickDesc}', async function() {`, `  const element = ${selector};`, `  await element.waitFor({ state: 'visible', timeout: 10000 });`, `  await element.click();`, needsWait
                ...
                case 'fill':
                    ...
                    if (!generatedPatterns.has(fillPattern)) {
                        ...
                        stepDefs.push(`When('I enter {string} into ${fillDesc}', async function(value: string) {`, `  const element = ${selector};`, `  await element.waitFor({ state: 'visible', timeout: ${isPassword ? 20000 : 10000} });`, `  await element.click();`, `  await element.fill(value);`, `  await page.waitForTimeout(300);`, `});`, ``);
                    }
                    break;
            }
        });
    ...
    shouldWaitForNavigation(action, nextAction) {
    ...
    convertSelectorToCode(selector) {
...
//# sourceMappingURL=recordingToGherkin.js.map