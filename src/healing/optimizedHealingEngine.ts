...
export interface HealingConfig {
  ...
  maxRetries: number;
  fastMode: boolean; // Skip fallbacks, go straight to vision
}
...
export class OptimizedHealingEngine {
  constructor(
    ...
    private visionHealer?: VisionHealer
  ) {}
  ...
  async findElement(context: {
    ...
    throw new Error(
      `Element not found: ${context.primarySelector}\n` +
   ...
   */
  private async useVisionHealing(context: {
    primarySelector: string;
    description: string;
  }): Promise<Locator> {
    ...
    try {
      const visionResult = await this.visionHealer.findElementWithVision(
        this.page,
        context.description || context.primarySelector,
        `Find element: ${context.primarySelector}`
      );
      ...
      const duration = ((Date.now() - start) / 1000).toFixed(1);
      console.error(`  ✗ Vision healing failed (${duration}s):`, 
   ...
   */
  async typeIntoElement(selector: string, text: string, description?: string): Promise<void> {
    const element = await this.findElement({
      primarySelector: selector,
      description: description || selector
    });
